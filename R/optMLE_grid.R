...
#' @param window_mult Window multiplier for how wide the grid should look on either side of the previous iteration's optimal design. The multiplier applies to the \code{audit_steps}; for example \code{window_mult = 3} (the DEFAULT) will look at a window of 3 times each step around the previous grid's optimal design.
...
#' @export
optMLE_grid <- function(phI, phII, phI_strat, min_n, window_mult = 3, audit_steps = c(40, 20, 10, 5, 1),
                        Y_unval, Y_val, X_unval, X_val, indiv_score, errors_in) {
  ...
  for (step in 2:length(audit_steps_prop)) {
    ...
    if (skippedLast) {
      # Run new grid
      new_grid <- prop_grid(prop_min = pmax(0, prev_grid_allocated / n_to_allocate - window_mult * audit_steps_prop[step - 1]),
                            prop_max = pmin(1, prev_grid_allocated / n_to_allocate + window_mult * audit_steps_prop[step - 1]),
      ...
      # Run new grid
      new_grid <- prop_grid(prop_min = pmax(0, prev_grid_allocated / n_to_allocate - window_mult * audit_steps_prop[step]),
                            prop_max = pmin(1, prev_grid_allocated / n_to_allocate + window_mult * audit_steps_prop[step]),
                            prop_inc = audit_steps_prop[step],
                            phII = phII,
                            phI_strat = phI_strat,
                            min_n = min_n,
    ...
    if (nrow(new_grid) > 0) {
      ...
      if (findOptimal) {
        all_opt_des[step, c("n00", "n01", "n10", "n11", "pi00", "pi01", "pi10", "pi11", "Vbeta")] <-  prev_min
        new_grid <- new_grid[order(new_grid$Vbeta),]
        prev_min <- new_grid[1, c("n00", "n01", "n10", "n11", "pi00", "pi01", "pi10", "pi11", "Vbeta")]
