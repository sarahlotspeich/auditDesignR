#' Score vectors for measurement error settings with errors in outcome only
#' @name score_Yonly
...
score_Yonly <- function(comp_dat, Y_val, Y_unval, X_val, Validated, beta, eta) {
  ...
  ## S(gamma0), ... , S(gamma3)
  mu2 <- data.matrix(cbind(1, Si_theta[, c(Y_val, X_val)])) %*% matrix(gamma_Ystar, ncol = 1)
  ...
  joint_v <- pY[val] * pYstar[val] * pX[val]
  joint_uv <- pY[-val] * pYstar[-val] * pX[-val]

  # Save the individual scores of validated subjects
  Si_theta_v <- Si_theta[val, ]

  # Unvalidated
  Si_theta$PhI_strat <- paste0("(", Si_theta[, Y_unval], ",", Si_theta[, X_val], ")")
  ...
  Si_theta_uv$Si_beta <- rowsum(x = d_beta[-val] * pYstar[-val] * pX[-val], group = Si_theta[-val, "PhI_strat"], reorder = F) / denom
  Si_theta_uv$Si_alpha <- rowsum(x = d_alpha[-val] * pYstar[-val] * pX[-val], group = Si_theta[-val, "PhI_strat"], reorder = F) / denom
  ...
  Si_theta_uv$Si_gamma3 <- rowsum(x = pY[-val] * pYstar[-val] * d_gamma3[-val], group = Si_theta[-val, "PhI_strat"], reorder = F) / denom
