###########################################################
# Outcome only misclassification ##########################
###########################################################
...
Ystar <- rbinom(n = N, size = 1, prob = sigmoid(eta[2] + eta[3] * Y + eta[4] * X))
sim_dat <- data.frame(Y, X, Ystar)

# Cross-tabulate Phase I strata -----------------------------
N00 <- table(Ystar, X)[1,1]
N01 <- table(Ystar, X)[1,2]
N10 <- table(Ystar, X)[2,1]
...
# Design 4: optMLE
grid_search <- optMLE_grid(phI = N, phII = n, phI_strat = stratN, min_n = 10, sample_on = c("Ystar", "X"), indiv_score = s)
...
wave1_strat <- data.frame(n00 = with(sim_dat[V_wave1 == 1, ], table(Ystar, X))[1, 1],
                          n01 = with(sim_dat[V_wave1 == 1, ], table(Ystar, X))[1, 2],
                          n10 = with(sim_dat[V_wave1 == 1, ], table(Ystar, X))[2, 1],
                          n11 = with(sim_dat[V_wave1 == 1, ], table(Ystar, X))[2, 2])
mle_wave1 <- twophase_mle(dat = cbind(V = V_wave1, sim_dat), Y_val = "Y", Y_unval = "Ystar", X_val = "X", X_unval = NULL, Validated = "V")
beta_hat <- mle_wave1$mod_Y_val$Est[2]
eta_hat <- with(mle_wave1, c(mod_Y_val$Est[1], mod_Y_unval$Est, mod_X_val$Est))
s_hat <- score(comp_dat = complete_data, Y_val = "Y", Y_unval = "Ystar", X_val = "X", X_unval = NULL, Validated = "V", beta = beta_hat, eta = eta_hat)
grid_search <- optMLE_grid(phI = N, phII = (n / 2), phI_strat = stratN, phIIa_strat = wave1_strat, min_n = 0, sample_on = c("Ystar", "X"), indiv_score = s_hat)
if (grid_search$findOptimal) {
